...
export default function ChatLayout() {
  ...
  // ── Charger les contacts depuis la BDD ──
  useEffect(() => {
    axios
  ...
  // ── Charger les messages du contact actif ──
  const loadMessages = useCallback(
    async (contact) => {
      ...
      try {
        let data;
        if (contact.isGlobal) {
          ({ data } = await axios.get(`${API}/messages/global`));
        } else {
          ({ data } = await axios.get(`${API}/messages/private/${contact.id}`));
        }
