...
CREATE TABLE users (
  ...
  CONSTRAINT chk_student_email CHECK (
    (
      role = 'student'
      AND email ~ '^hei\.[a-zA-Z0-9._%+-]+@gmail\.com$'
    )
...
CREATE TABLE posts (
  ...
  author_id INTEGER NULL REFERENCES users(id) ON DELETE
  SET
    NULL,
    created_at TIMESTAMP NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMP NOT NULL DEFAULT NOW(),
...
CREATE TABLE supports (
  ...
  SET
    NULL,
    created_at TIMESTAMP NOT NULL DEFAULT NOW(),
    CONSTRAINT chk_support_ue CHECK (
      ue IN (
        'WEB1',
        'WEB2',
        'WEB3',
...
CREATE TABLE submissions (
  id SERIAL PRIMARY KEY,
  student_id INTEGER NULL REFERENCES users(id) ON DELETE
  SET
    NULL,
    nom VARCHAR(100) NOT NULL,
    prenom VARCHAR(100) NOT NULL,
    ...
    CONSTRAINT chk_groupe_level CHECK (
      (
        level = 'L1'
        AND groupe IN ('N1', 'N2', 'N3', 'N4')
      )
      OR (
        level = 'L2'
        AND groupe IN ('K1', 'K2', 'K3')
      )
      OR (
        level = 'L3'
        AND groupe IN ('J1', 'J2')
...
CREATE TABLE messages (
  id SERIAL PRIMARY KEY,
  sender_id INTEGER NULL REFERENCES users(id) ON DELETE
  SET
    NULL,
    receiver_id INTEGER NULL REFERENCES users(id) ON DELETE
  SET
    NULL,
...
UPDATE
  ON posts FOR EACH ROW EXECUTE FUNCTION fn_set_updated_at();
