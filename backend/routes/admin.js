...
// Accepte `failed_refs` (ancienne UI) ou `failed_l1_refs`+`failed_l2_refs` (nouvelle UI)
router.post("/class-upgrade", auth, adminOnly, async (req, res) => {
  const { failed_refs, failed_l1_refs, failed_l2_refs } = req.body;
  const allFailed = failed_refs || [...(failed_l1_refs || []), ...(failed_l2_refs || [])];
  try {
    const result = await db.query(
