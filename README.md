...
- **Admin** — User management (search, role/email change, delete), invitation management (single + bulk up to 1000), stats dashboard with live polling, class upgrade (L1→L2→L3→alumni with failed refs), L1 registration (November), real-time user updates via Socket.IO
